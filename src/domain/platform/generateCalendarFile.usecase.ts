...
export class GenerateCalendarFileUsecase {
  ...
  /**
   * Generate an iCalendar (.ics) file content for a user's recurring daily reminder
  ...
  async generate(userId: string): Promise<string> {
    ...
    if (!user.reminderTime) {
      throw new NotFoundException('No reminder configured for this user');
    }

    return this.generateICalContent(userId, user.reminderTime);
  }
  ...
  private generateICalContent(userId: string, reminderTime: string): string {
    ...
    // Stable UID based on userId - allows calendar apps to update the event
    const uid = `deepmobility-reminder-${userId}@deepmobility.fr`;
    const dtstamp = formatDate(now);
    const dtstart = formatDate(startDate);
    const dtend = formatDate(endDate);

    // Get personalized emoji and message based on time
